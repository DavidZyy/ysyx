...
/**
 * input: s, type
 * output(if instruction type have): dest, src1, src2, imm
 */
...
/* 2.4 Integer Computational Instructions */
  ...
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> RV64_shamt(imm));
...
/* RV64I */
  ...
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , R, R(dest) = SEXT(BITS(src1 - src2, 31, 0), 32)); /* the src1 = R(rs1), see decode_operand */
  

  /* 5.3 Load and Store Instructions */
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(BITS(Mr(src1 + imm, 2), 31, 0), 16));
